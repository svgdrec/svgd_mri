...
class MriModule(pl.LightningModule):
    ...
    def validation_epoch_end(self, val_logs):
        ...
        print('SSIM/NMSE', metrics["ssim"], metrics["nmse"])
    ...
    def test_epoch_end(self, test_logs):
        ...
        for log in test_logs:
            for i, (fname, slice_num) in enumerate(zip(log["fname"], log["slice"])):
                outputs[fname][int(slice_num.cpu())] = log["output"][i].cpu().numpy()
                #outputs_std[fname][int(slice_num.cpu())] = log["std"][i]
                target = log["target"][i].cpu().numpy()
                
                #print(evaluate.ssim(target[None, ...], outputs[fname][int(slice_num.cpu())][None, ...], maxval=log["max_value"][i].cpu().numpy())) #evaluate.mse(target, outputs[fname][int(slice_num.cpu())]))
        ...
        for fname in outputs:
            ...
            #outputs_std[fname] = np.stack(
            #    [out for _, out in sorted(outputs_std[fname].items())]
            #)
        ...
        #fastmri.save_reconstructions(outputs_std, pathlib.Path(self.trainer.default_root_dir) / 'std')
