...
class UnetModule(MriModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.loss_f = torch.nn.GaussianNLLLoss(eps=1e-02)
        self.splus = torch.nn.Softplus() #torch.nn.ReLU()
    ...
    def training_step(self, batch, batch_idx):
        ...
        var = self.splus(output[:,1].unsqueeze(1))#torch.pow(,2)

        #square = torch.pow(mu - target.unsqueeze(1),2)## preserve the same shape as y_pred.shape
        #loss = torch.divide(square,var) + torch.log(var)

        loss = self.loss_f(mu, batch.target.unsqueeze(1), var)

        #loss1 = torch.pow(mu-batch.target.unsqueeze(1),2) #F.l1_loss(mu, batch.target.unsqueeze(1))
        #print('LOSS!', loss1)
        #print('sigma!', sigma)
        #print('logsigma!', torch.log(sigma))
        #exit()
        #loss = torch.mean(loss1/sigma + 0.5 * torch.log(sigma))
    ...
    def test_step(self, batch, batch_idx):
        output = self(batch.image)
        mu = output[:,0]
        var = torch.exp(output[:,1].unsqueeze(1))
